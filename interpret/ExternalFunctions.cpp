...
//  Interpret.  The first is to implement lle_* wrapper functions that are
...
//  not exist, and libffi is available, then the Interpret will attempt to
...
static bool ffiInvoke(RawFunc Fn, Function *F, ArrayRef<GenericValue> ArgVals,
  ...
  if (ArgVals.size() > NumArgs && F->isVarArg()) {
    report_fatal_error("Calling external var arg function '" + F->getName()
                      + "' is not supported by the Interpret.");
  }
...
GenericValue Interpret::callExternalFunction(Function *F,
...
static GenericValue lle_X_atexit(FunctionType *FT,
  ...
  assert(Args.size() == 1);
  TheInterpret->addAtExitHandler((Function*)GVTOP(Args[0]));
  GenericValue GV;
  GV.IntVal = 0;
  return GV;
}
...
static GenericValue lle_X_exit(FunctionType *FT, ArrayRef<GenericValue> Args) {
  TheInterpret->exitCalled(Args[0]);
  return GenericValue();
}
    ...
    case '%': {                    // Handle format specifiers
      ...
      *FB = 0;
      ...
      case 'x': case 'X':
        if (HowLong >= 1) {
          if (HowLong == 1 &&
              TheInterpret->getDataLayout().getPointerSizeInBits() == 64 &&
...
void Interpret::initializeExternalFunctions() {
  sys::ScopedLock Writer(*FunctionsLock);
